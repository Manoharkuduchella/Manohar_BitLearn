...
int main(void)
{
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  ...
	  BME280_Measure((float *)&sensor_log.temperature, (float *)&sensor_log.pressure, (float *)&sensor_log.humidity);

	  //printf("Writing T: %.2f\r\n",
	              //sensor_log.temperature);

	  // wrap sector
	  if((write_addr + sizeof(bme_log_t)) >
	       (FLASH_START_ADDR + FLASH_SECTOR_SIZE))
	  {
	      //printf("Flash Wrapped\r\n");
