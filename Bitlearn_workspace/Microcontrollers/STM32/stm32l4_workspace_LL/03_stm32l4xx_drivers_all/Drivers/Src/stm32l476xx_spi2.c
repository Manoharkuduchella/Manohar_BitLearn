...
void SPI_Init(SPI_Handle_t *pSPIHandle) {
	...
	if (pSPIHandle->SPI_Config.DFF == SPI_DFF_8BITS)
	{
		...
		pSPIHandle->pSPIx->CR2 |= (1 << SPI_CR2_FRXTH);   // RXNE at 8-bit
	}
	else
	{
		pSPIHandle->pSPIx->CR2 |= (15 << SPI_CR2_DS);  // 16-bit
		pSPIHandle->pSPIx->CR2 &= ~(1 << SPI_CR2_FRXTH);  // RXNE at 16-bit
	}
...
void SPI_SendData(SPI_RegDef_t *pSPIx, uint8_t *pTxBuffer, uint32_t Len)
{
	...
	{
		/* Wait until TXE = 1 */
		while ((pSPIx->SR & SPI_TXE_FLAG) == 0);

		/* 8-bit mode */
		if (((pSPIx->CR2 >> SPI_CR2_DS) & 0xF) == 0x7)
		{
		...
		/* 16-bit mode */
		else if (((pSPIx->CR2 >> SPI_CR2_DS) & 0xF) == 0xF)
		{
