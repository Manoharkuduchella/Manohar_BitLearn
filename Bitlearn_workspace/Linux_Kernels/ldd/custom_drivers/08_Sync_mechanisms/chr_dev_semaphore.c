...
//DEFINE_SEMAPHORE(ch_semaphore);//static

struct semaphore *ch_semaphore;
...
ssize_t ch_read(struct file *filp, char __user *buff, size_t count, loff_t *f_pos)
{
	...
	pr_info("%s : acquiring the C.S with semaphore\n",__func__);

	down(ch_semaphore);//decrements the counter
	pr_info("%s : Down sem count value %d\n",__func__,ch_semaphore->count);
	...
	*f_pos += count;
	...
	up(ch_semaphore);//increment the counter
	pr_info("%s : Up sem count value %d\n",__func__,ch_semaphore->count);

	pr_info("%s : releasing the critical section with semaphore\n",__func__);
...
static int __init chr_in(void)
{
	...
	/*Init semaphore*/
	ch_semaphore = kzalloc(sizeof(struct semaphore),GFP_KERNEL);
	if(!ch_semaphore)
	{
		pr_info("%s : No Memory\n",__func__);
		return -ENOMEM;
	}
	sema_init(ch_semaphore,1);
	pr_info("semaphore init done\n");
