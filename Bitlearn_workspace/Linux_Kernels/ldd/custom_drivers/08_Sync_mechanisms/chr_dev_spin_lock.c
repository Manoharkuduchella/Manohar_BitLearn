...
static DEFINE_SPINLOCK(ch_spin_lock);//static spinlock initialization
...
ssize_t ch_read(struct file *filp, char __user *buff, size_t count, loff_t *f_pos)
{
	...
	pr_info("%s : acquiring the spin lock on processor %d\n",__func__,smp_processor_id());


	spin_lock(&ch_spin_lock);
	...
	*f_pos += count;
	...
	spin_unlock(&ch_spin_lock);

	pr_info("%s : releasing the spin lock on processor %d\n",__func__,smp_processor_id());
