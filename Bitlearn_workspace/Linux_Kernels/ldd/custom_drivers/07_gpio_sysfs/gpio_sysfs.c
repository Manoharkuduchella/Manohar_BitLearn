...
ssize_t direction_store(struct device *dev, struct device_attribute *attr,const char *buf, size_t count)
{
    ...
    /*compare sysfs strings and update attribute*/
...
int gpio_sysfs_probe(struct platform_device *pdev)
{
    ...
    {
        dev_err(dev,"%s : No child dt devices found\n",__func__);
        return -EINVAL;
    }

    dev_info(dev,"Total device(Child) found = %d\n",gpio_drv_data.total_devices);
    ...
    for_each_available_child_of_node(parent, child)
    {
        ...
        /*read/get the gpio pins info form "bone" function*/
        ...
        /*create devices under /sys/class/bone_gpios and also create sysfs attributes for each device*/
        gpio_drv_data.dev[i] = device_create_with_groups(gpio_drv_data.class_gpio, dev, 0,
                                    dev_data, gpio_attr_groups,
                                    dev_data->label);
